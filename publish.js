...
function generateSourceFiles(sourceFiles, encoding) {
    ...
    Object.keys(sourceFiles).forEach(function(file) {
        ...
        generate(
            "Source",
            sourceFiles[file].shortened, [source],
            sourceOutfile,
            false
        )
    })
...
function buildNav(members) {
    ...
    nav.push(buildNavLink('home', '<a href="index.html">' + env.opts.title + '</a>'))
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    function generateTutorial(title, tutorial, filename, prevTutorial, nextTutorial) {
        var tutorialData = {
            title: title,
    ...
    function saveChildren(node) {
        node.children.forEach(function(child, index, array) {
            var prevTutorial = nextTutorial = {};
            if (index > 0) {
                prevTutorial = array[index - 1];
            }
            if (index < array.length - 1) {
                nextTutorial = array[index + 1];
            }
            generateTutorial(
                child.title,
                child,
